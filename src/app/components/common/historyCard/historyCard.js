...
export default function HistoryCard({ item, onPress, isHistory }) {
  ...
  const price = item?.best_price || product?.best_price || 0;


  const handleDelete = () => {
    Alert.alert(
      isHistory ? "Delete History" : "Remove from Favorites",
      isHistory
  ...
  return (
    <View style={styles.card}>
      <TouchableOpacity activeOpacity={1} onPress={onPress} style={styles.cardContent}>
      ...
      <TouchableOpacity onPress={handleDelete} style={styles.actionButton}>
        {isHistory ? (
          <Trash2 size={Responsive.width(20)} color={colors.error || "#FF3B30"} />
        ) : (
          <Heart size={Responsive.width(20)} color={colors.error || "#FF3B30"} fill={colors.error || "#FF3B30"} />
        )}
      </TouchableOpacity>
    </View>
  )
